...
const cronJobs = [
	...
	new cron('0 0 0 * * 6', (() => { pullFeedsInterval(60 * 24 * 7); }), null, false), // one week
];
...
plugins.isActive('nodebb-plugin-rss-brad', (err, active) => {
	if (err) {
...
pubsub.on('nodebb-plugin-rss-brad:deactivate', () => {
	stopCronJobs();
});
