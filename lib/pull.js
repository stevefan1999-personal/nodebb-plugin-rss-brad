...
async function postEntry(feed, entry) {
	...
	if (Array.isArray(entry.tags)) {
		const entryTags = entry.tags.filter(Boolean);
	...
	if (!htmlContent) {
		try {
			...
			htmlContent = $('.entry-content.gridmode-clearfix').html(); // Adjust this selector based on the actual structure
		} catch (error) {
			winston.error(`[plugin-rss-brad] failed to fetch webpage content, ${entry.link.href}`, error);
			htmlContent = ''; // Fallback content or handle the error appropriately
		}
	}
	...
	try {
		...
		winston.error(`[plugin-rss-brad] failed to turn down HTML content`, error);
        content = undefined; // Fallback content or handle the error appropriately
	}
