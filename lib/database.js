...
Database.getFeeds = async function () {
	...
	const keys = feedUrls.map(url => `nodebb-plugin-rss-brad:feed:${url}`);
...
Database.saveFeeds = async function saveFeeds(feeds) {
	...
	async function saveFeed(feed) {
		await Promise.all([
			db.setObject(`nodebb-plugin-rss-brad:feed:${feed.url}`, feed),
			db.setAdd('nodebb-plugin-rss-brad:feeds', feed.url),
		]);
	}
...
Database.deleteFeeds = async function deleteFeeds() {
	const feeds = await db.getSetMembers('nodebb-plugin-rss-brad:feeds');
	if (!feeds.length) {
		return;
	}

	const keys = feeds.map(feed => `nodebb-plugin-rss-brad:feed:${feed}`);
	await db.deleteAll(keys);
	await db.setRemove('nodebb-plugin-rss-brad:feeds', feeds);
};
